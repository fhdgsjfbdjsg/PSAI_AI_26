...
# 2. ДАННЫЕ (AND^i — инверсное AND)
X = np.array(list(product([0, 1], repeat=n)))
y = np.array([int(not all(x)) for x in X])
...
def train_model(X_train, y_train, X_test, y_test, lr=0.1, adaptive=False):
    ...
    for epoch in range(max_epochs):
        ...
        # балансировка классов (усилена из-за сильного дисбаланса)
        class_weights = np.where(y_train == 0, 50, 1)
...
# 8. ГРАФИКИ
plt.figure()
plt.plot(train_f, label="Обучающая (фиксированный шаг)")
plt.plot(test_f, label="Тестовая (фиксированный шаг)")
plt.plot(train_a, label="Обучающая (адаптивный шаг)")
plt.plot(test_a, label="Тестовая (адаптивный шаг)")

plt.xlabel("Номер эпохи")
plt.ylabel("Ошибка (Cross-Entropy)")
plt.title("Анализ сходимости обучения (AND^i)")
plt.legend()
plt.grid()
plt.show()
...
while True:
    ...
    if len(x) != n or any(i not in [0,1] for i in x):
        print(f"Ошибка: нужно ввести ровно {n} значений 0 или 1")
        continue
